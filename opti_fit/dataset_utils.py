...
CUTOFF_THRESHOLDS = {Algorithm.REGEX_MATCH: 90,
...
def _validate_dataset(df: pd.DataFrame) -> None:
    ...
    assert set(df.columns) == {'payment_case_id', ' Fuzz Partial Token Sort Ratio', ' Fuzz Token Sort Ratio', ' Fuzz Ratio', ' Fuzz Partial Ratio', ' Jaro Winkler', ' is_payment_true_hit', ' is_hit_true_hit', ' Regex Match'}
