...
def run_solver_comparison(model_type: str, n_seeds: int, dataset: str, to_file: bool):
    model_name = "simple_" + model_type
    solvers = ["GUROBI", "CBC", "HIGHS"]
    model, df = parse_and_validate_runner_input(model_name=model_name, dataset=dataset)
    ...
    for solver_name in tqdm(solvers):
        for seed in range(n_seeds):
            ...
            result, columns = merge_performance_and_cutoff_output(performance_df, cutoffs)
            results.append((solver_name, seed, runtime) + result)

    columns = ["solver_name", "seed", "runtime"] + columns
